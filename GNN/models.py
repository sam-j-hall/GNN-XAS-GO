...
class GNN_atom(torch.nn.Module):
    '''
        Graph neural network class to run various types of GNNs
    '''
    def __init__(self, num_tasks, num_layers=4, in_channels=[33,100,100,100], out_channels=[100,100,100,100],
    ...
    def forward(self, batched_data):
        ...
        graph_sizes = torch.bincount(batched_data.batch)
        ...
        h_weight = 0.5 * graph_embedding
        h_new = h_weight + node_embedding
        # Activation function
        p = torch.nn.LeakyReLU(0.1)
        # Pass through linear layer
        out = p(self.graph_pred_linear(node_embedding))
